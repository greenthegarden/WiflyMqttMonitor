...
qos = 2
topics = []
topics.append(("monitor/status/#", 0))
topics.append(("security/sensor/#", 0))
...
command_types_valid_inputs = ['s']
sensor_valid_states = ['0', '1', '2', '3']

print("Enter 's' for sensor: ")
...
while True :
	try :
		...
		if command_type in command_types_valid_inputs :
			if command_type is 's' :
				sensor_state = raw_input("Enter 0/1/2/3 to set state: ")
				if sensor_state in sensor_valid_states :
					messageStr = "0" + "," + sensor_state
					mqttc.publish("homesecurity/status/sensor", messageStr);
			else :
